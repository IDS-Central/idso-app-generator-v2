...
export const iamCreateSa: ToolHandler<IamCreateSaInput, IamCreateSaOutput> = async (
  ...
  // We do get+modify+set in a single RPC cycle. If another writer races us,
  // setIamPolicy will fail with 409 and the caller can retry.
  const rolesAlreadyBound: string[] = [];
  try {
    ...
    if (changed) {
      ...
      log.info(
        { event: 'iam_roles_granted', roles: RUNTIME_ROLES, roles_already_bound: rolesAlreadyBound },
        'project IAM roles granted',
      );
    } else {
      log.info(
        { event: 'iam_roles_noop', roles_already_bound: rolesAlreadyBound },
